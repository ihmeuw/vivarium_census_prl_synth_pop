...
class DOBGenerator(GenericGenerator):
    ...
    def noise(
        self,
        df,
        pr_field_error=0.0106,
        pr_full_error=0.0026,
        pr_missing=0.0024,
        pr_month_day_swap=0.0018,
    ):
        ...
        for col in ["year", "month", "day"]:
            ...
            df.loc[rows, col] = df.loc[rows, col] + np.random.choice(
                [-2, -1, 1, 2], size=np.sum(rows, dtype=int)
            )  # TODO: investigate error distribution for these errors (current approach is not evidence-based)
        df.month = np.clip(df.month, 1, 12, dtype=int)
        df.day = np.clip(
            df.day, 1, 31, dtype=int
        )  # NOTE: it is acceptible to have an error that has a day of 31 in a month with less days (because it is an erroneous DOB)
