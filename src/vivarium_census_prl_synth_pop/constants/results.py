...
STANDARD_COLUMNS = {"total_population": TOTAL_POPULATION_COLUMN}


TOTAL_POPULATION_COLUMN_TEMPLATE = "total_population_{POP_STATE}"

COLUMN_TEMPLATES = {"population": TOTAL_POPULATION_COLUMN_TEMPLATE}
...
TEMPLATE_FIELD_MAP = {"POP_STATE": POP_STATES, "YEAR": YEARS, "SEX": SEXES}
...
def RESULTS_MAP(kind):
    ...
    fields, value_groups = (
        list(filtered_field_map.keys()),
        list(itertools.product(*filtered_field_map.values())),
    )
    for value_group in value_groups:
        columns.append(
            template.format(**{field: value for field, value in zip(fields, value_group)})
